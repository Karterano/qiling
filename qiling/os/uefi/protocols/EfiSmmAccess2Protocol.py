...
class EFI_SMM_ACCESS2_PROTOCOL(STRUCT):
    ...
    def hook_GetCapabilities(ql: Qiling, address: int, params):
        ...
        chunks = EFI_SMM_ACCESS2_PROTOCOL._coalesce(chunks)
...
descriptor = {
    ...
    "fields" : (
        ("Open",            EFI_SMM_ACCESS2_PROTOCOL.hook_Open),
        ("Close",            EFI_SMM_ACCESS2_PROTOCOL.hook_Close),
        ("Lock",            EFI_SMM_ACCESS2_PROTOCOL.hook_Lock),
        ("GetCapabilities",    EFI_SMM_ACCESS2_PROTOCOL.hook_GetCapabilities)
    )
}
